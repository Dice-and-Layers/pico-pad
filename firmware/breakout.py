...
def run_game(display, get_keys):
    ...
    score, state = 0, "START"
    high_score = utils.get_high_score("breakout")
    while True:
        keys = get_keys()
        if (0, 0) in keys and (0, 2) in keys: return
        if state == "START":
            display.fill(0); utils.draw_text(display, "BREAKOUT", 25, 15, scale=2)
            ...
            if b_y > HEIGHT or not any(b[2] for b in bricks):
                state = "GAMEOVER"; utils.save_high_score("breakout", score); high_score = utils.get_high_score("breakout"); time.sleep(0.5)
            display.fill(0); display.fill_rect(p_x, HEIGHT - PADDLE_H, PADDLE_W, PADDLE_H, 1); display.hline(p_x, HEIGHT - PADDLE_H + 1, PADDLE_W, 0)
