...
def run_game(display, get_keys):
    """Main game loop for Winged Jump."""
    ...
    while True:
        ...
        if state == "START":
            display.fill(0); utils.draw_text(display, "WINGED JUMP", 25, 15, scale=2)
            ...
            if bird_y < 0 or bird_y > HEIGHT - BIRD_SIZE: state = "GAMEOVER"; utils.save_high_score("winged_jump", score); high_score = utils.get_high_score("winged_jump"); time.sleep(0.5)
            ...
            for p in pipes:
                ...
                if p[0] < 20 + BIRD_SIZE and p[0] + PIPE_W > 20:
                    if bird_y < p[1] or bird_y + BIRD_SIZE > p[1] + GAP:
                        state = "GAMEOVER"; utils.save_high_score("winged_jump", score); high_score = utils.get_high_score("winged_jump"); time.sleep(0.5)
                if p[0] == 20: score += 1
