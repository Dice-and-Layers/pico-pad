...
def run_game(display, get_keys):
    ...
    high_score = utils.get_high_score("snake")

    while True:
        keys = get_keys()
        if (0, 0) in keys and (0, 2) in keys: return
        if state == "START":
            display.fill(0); utils.draw_text(display, "SNAKE", 40, 15, scale=2)
            ...
            if time.monotonic() - last_move > speed:
                ...
                if head[0] < 0 or head[0] >= WIDTH//BLOCK or head[1] < 0 or head[1] >= HEIGHT//BLOCK or head in snake:
                    state = "GAMEOVER"; utils.save_high_score("snake", score); high_score = utils.get_high_score("snake"); time.sleep(0.5)
                else:
                    snake.insert(0, head)
                    if head == food:
            ...
            utils.draw_text(display, f"SCORE: {score}", 35, 30, scale=1)
            utils.draw_text(display, f"HIGH: {high_score}", 35, 42, scale=1)
