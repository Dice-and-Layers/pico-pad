...
class Paddle:
    def __init__(self, x):
        self.x, self.y = x, HEIGHT // 2 - PADDLE_H // 2
        self.score = 0
    def move(self, dy):
...
def run_game(display, get_keys):
    p1, p2, ball = Paddle(1), Paddle(WIDTH - 1 - PADDLE_W), Ball()
    state = "START"
    while True:
        keys = get_keys()
        if (0, 0) in keys and (0, 2) in keys: return
        if state == "START":
            display.fill(0); utils.draw_text(display, "PONG", 45, 15, scale=2)
            ...
            utils.draw_text(display, str(p1.score), 40, 5); utils.draw_text(display, str(p2.score), 80, 5)
            display.show()
        elif state == "GAMEOVER":
            display.fill(0); winner = "P1 WINS" if p1.score >= 5 else "P2 WINS"
            utils.draw_text(display, winner, 35, 25, scale=2); display.show()
