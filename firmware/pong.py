...
class Ball:
    ...
    def reset(self): self.x, self.y, self.vx, self.vy = WIDTH // 2, HEIGHT // 2, (2.5 if random.random() > 0.5 else -2.5), (random.random() - 0.5) * 4
...
def run_game(display, get_keys):
    ...
    high_score = utils.get_high_score("pong")
    while True:
        keys = get_keys()
        ...
        if state == "START":
            ...
            if p1.score >= 5 or p2.score >= 5:
                state = "GAMEOVER"; utils.save_high_score("pong", max(p1.score, p2.score)); high_score = utils.get_high_score("pong")
