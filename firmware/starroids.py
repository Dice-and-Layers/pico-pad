...
class Entity:
    ...
    def __init__(self, x, y, size): 
        self.x, self.y, self.size, self.vx, self.vy = x, y, size, 0, 0
    def update(self): 
        # Wrap around screen edges
        self.x, self.y = (self.x + self.vx) % WIDTH, (self.y + self.vy) % HEIGHT

class Ship(Entity):
    """The player's starship."""
    def __init__(self): 
        super().__init__(WIDTH // 2, HEIGHT // 2, 8); self.angle = 0
    def rotate(self, da): self.angle += da
    def accelerate(self): 
        self.vx += math.cos(self.angle) * 0.4; self.vy += math.sin(self.angle) * 0.4
    def update(self): 
        super().update()
...
class Hazard(Entity):
    """Floating space hazards (formerly asteroids)."""
...
def run_game(display, get_keys):
    ...
    while True:
        ...
        if state == "START":
            display.fill(0); utils.draw_text(display, "STARROIDS", 30, 15, scale=2)
            utils.draw_text(display, f"HI:{high_score}", 50, 35); utils.draw_text(display, "CENTER TO START", 35, 50); display.show()
            if (1, 1) in keys: state, score, ship, bullets, hazards = "PLAYING", 0, Ship(), [], [Hazard() for _ in range(3)]; time.sleep(0.3)
            ...
            if (1, 1) in keys and time.monotonic() - last_fire > 0.3: 
                bullets.append(Bullet(ship.x, ship.y, ship.angle)); last_fire = time.monotonic()
            ...
            for h in hazards:
                ...
                if check_collision(ship, h, 6):
                    state = "GAMEOVER"; utils.save_high_score("starroids", score); high_score = utils.get_high_score("starroids")
            
            for b in bullets[:]:
                for h in hazards[:]:
                    if check_collision(b, h, 6):
                        if b in bullets: bullets.remove(b)
                        hazards.remove(h); score += 10
